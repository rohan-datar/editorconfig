...
Emacs' default text shaping doesn't
ask HarfBuzz to compose multi-character sequences, so the font never
gets a chance to substitute them. =ligature.el= fixes that by
registering prefix sequences with =composition-function-table= so
HarfBuzz sees them as a unit and can apply the font's GSUB rules.
