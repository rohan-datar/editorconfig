...
- [[#org-mode][Org Mode]]
  ...
  -  [[#org-knowledge-base][Org Knowledge Base]]
...
The Nix wrapper sets ~user-emacs-directory~ to =~/.cache/emacs=, which
handles most writable paths automatically. We only need to explicitly
redirect a few package-specific paths and ensure Nix binaries are on PATH.
#+begin_src emacs-lisp
    ;; Ensure cache directory exists (for package-specific paths below)
    (let ((cache-dir (expand-file-name "~/.cache/emacs/")))
      ...
      ;; Custom file: when nil, customize tries to write to the init file (read-only in Nix)
      (setq custom-file (expand-file-name "custom-vars.el" cache-dir))
      ;; Package-specific paths that don't derive from user-emacs-directory
      (setq undo-fu-session-directory (expand-file-name "undo-fu-session/" cache-dir))
      (setq shell-maker-root-path (expand-file-name "shell-maker/" cache-dir))
      (setq prescient-save-file (expand-file-name "prescient-save.el" cache-dir)))
...
Moved to =early-init.el= for proper effect before package initialization.
...
#+begin_src emacs-lisp
    (use-package evil
      ...
      (evil-set-initial-state 'dashboard-mode 'normal) ;; Set initial state in dashboard to normal
      ...
      ;; Tab navigation
      (evil-define-key 'normal 'global (kbd "] t") 'tab-bar-switch-to-next-tab)
      (evil-define-key 'normal 'global (kbd "[ t") 'tab-bar-switch-to-prev-tab)

      ;; move selected lines up/down via :move, then gv=gv to reindent
      ;; and restore the visual selection on the moved range
      (evil-define-key 'visual 'global (kbd "K") (kbd ": m '<-2 RET gv=gv"))
      (evil-define-key 'visual 'global (kbd "J") (kbd ": m '>+1 RET gv=gv"))

      ;; vim's :m updates '<,'> to follow the moved text so gv re-selects it.
      ;; evil-move only does this when called from active visual state, which
      ;; isn't true when invoked via : from visual mode. Patch it to always
      ;; update the visual markers so the gv=gv idiom works as in vim.
      (advice-add 'evil-move :around
                  (lambda (orig beg end address)
                    (let ((n (count-lines beg end)))
                      (funcall orig beg end address)
                      (move-marker evil-visual-point (line-end-position))
                      (move-marker evil-visual-mark
                                   (save-excursion (forward-line (- 1 n))
                                                   (point))))))
...
*** Evil Add-Ons
Extensions for evil-mode functionality
...
#+begin_src emacs-lisp
    (use-package general
      ...
      (rdmacs/leader-keys
        ...
        "f s" '(treemacs :wk "Toggle treemacs"))
      ...
      (rdmacs/leader-keys
        "t" '(:ignore t :wk "Tabs")
        "t t" '(tab-bar-switch-to-tab :wk "Switch tab")
        "t o" '(tab-bar-new-tab :wk "Open new tab")
    	"t i" '(tab-bar-list :wk "List tabs")
    	"t k" '(tab-bar-close-tab :wk "Close current tab")
      ...
      (rdmacs/leader-keys
    	...
    	"o p" '(ghostel-project :wk "Open project terminal")
    	"o o" '(ghostel-other :wk "Switch to other terminal"))


      (rdmacs/leader-keys
    	"v" '(:ignore t :wk "Visual")
    	"v l" '(visual-line-mode :wk "Toggle truncated lines (wrap)")
...
*** Ligatures
Emacs' default text shaping doesn't ask HarfBuzz to compose
multi-character sequences, so the font never gets a chance to
substitute them. =ligature.el= fixes that by registering prefix
sequences with =composition-function-table= so HarfBuzz sees them as a
unit and can apply the font's GSUB rules.

The sequences below are taken from Maple Mono's ligature feature spec.
HarfBuzz silently ignores any sequence the font doesn't actually
ligate, so over-registering is harmless.
#+begin_src emacs-lisp
  (use-package ligature
    ...
    (ligature-set-ligatures
     ...
     '(;; arrows
       ...
       ".." "..." "..<" ".?" ".="
       ;; tag ligatures
       "[TODO]" "[FIXME]" "[INFO]" "[WARN]" "[ERROR]"
       "[DEBUG]" "[TRACE]" "[FATAL]" "[NOTE]" "[HACK]" "[MARK]"))
...
#+begin_src emacs-lisp
    (use-package doom-modeline
      ...
      :config
      (doom-modeline-def-segment rdmacs-evil-state
        (let* ((state (or (and (bound-and-true-p evil-local-mode) evil-state)
                          'emacs))
               (face (intern (format "doom-modeline-evil-%s-state" state))))
          (propertize (format " %s " (upcase (symbol-name state)))
                      'face (cond ((not (doom-modeline--active)) 'mode-line-inactive)
                                  ((facep face) face)
                                  (t 'mode-line)))))

      ;; Mostly standard with some reordering
      (doom-modeline-def-modeline 'main
    	'(eldoc bar window-state workspace-name window-number
    			rdmacs-evil-state vcs check
    			matches follow buffer-info remote-host buffer-position
    			word-count parrot selection-info)
    	'(compilation objed-state misc-info project-name persp-name battery
...
#+begin_src emacs-lisp
    (use-package golden-ratio
      ...
      :custom
      (golden-ratio-exclude-buffer-regexp '("^\\*claude"))
...
#+begin_src emacs-lisp
    (use-package lsp-mode
      ...
      (lsp-eldoc-render-all t)              ;; Render full documentation, not just signature
...
This mirrors the nvim-dap setup with support for Go and C/C++ via LLDB.
...
** Flyover
Show flymake errors in a nice virtual text interface
#+begin_src emacs-lisp
    (use-package flyover
...
(use-package eldoc
  :ensure nil                                ;; This is built-in, no need to fetch it.
  :custom
  (eldoc-idle-delay 0.3)                     ;; Avoid expensive doc refreshes on every keystroke
  (eldoc-display-functions '(eldoc-display-in-echo-area eldoc-display-in-buffer)) ;; Keep *eldoc* buffer for `eldoc-box-help-at-point`
  (eldoc-echo-area-use-multiline-p nil)      ;; We use the "K" floating help instead
...
*** LaTeX
AUCTeX provides structure-aware LaTeX editing and smart compilation; RefTeX
handles labels and citations; PDF Tools embeds a fast PDF viewer with SyncTeX
source <-> PDF sync. latexmk, chktex and texlab come from packages.nix
(runtimeDeps). Compile with C-c C-c, view with C-c C-v, insert environments
with C-c C-e, sections with C-c C-s, labels with C-c (, references with C-c )
and citations with C-c [.
#+begin_src emacs-lisp
    ;; The ELPA package is named "auctex", but its features are `tex'/`latex'.
    ;; Load eagerly so `LaTeX-mode' is defined and .tex files cannot fall back
    ;; to the built-in (non-AUCTeX) latex-mode.
    (use-package latex
      :demand t
      :mode ("\\.tex\\'" . LaTeX-mode)
      :hook ((LaTeX-mode . turn-on-reftex)            ; labels, refs, citations
             (LaTeX-mode . TeX-source-correlate-mode) ; SyncTeX
             (LaTeX-mode . flymake-mode))             ; chktex diagnostics
      :custom
      (TeX-auto-save t)                     ;; Cache style parse info
      (TeX-parse-self t)                    ;; Parse file on load to detect packages
      (TeX-source-correlate-start-server t) ;; Allow PDF -> source jumps
      (reftex-plug-into-AUCTeX t)           ;; RefTeX drives AUCTeX refs/citations
    ...
    ;; Live preview
    (defun rdmacs/latex-compile-on-save ()
      "Recompile the TeX master with latexmk after saving a LaTeX buffer."
      (when (derived-mode-p 'latex-mode)
        (TeX-command "LaTeXMk" 'TeX-master-file)))
    (add-hook 'LaTeX-mode-hook
              (lambda ()
                (add-hook 'after-save-hook
                          #'rdmacs/latex-compile-on-save nil t)))

    (rdmacs/leader-keys
      :keymaps 'LaTeX-mode-map
      "x" '(:ignore t :wk "LaTeX")
      "x c" '(TeX-command-master :wk "Run command (C-c C-c)")
      "x m" '(rdmacs/latex-compile-on-save :wk "Compile LaTeXMk now")
      "x v" '(TeX-view :wk "View PDF")
      "x n" '(TeX-next-error :wk "Next error")
      "x p" '(TeX-previous-error :wk "Previous error")
      "x e" '(LaTeX-environment :wk "Insert environment")
      "x s" '(LaTeX-section :wk "Insert section")
      "x (" '(reftex-label :wk "Insert label")
      "x )" '(reftex-reference :wk "Insert reference")
      "x [" '(reftex-citation :wk "Insert citation")
      "x b" '(TeX-recenter-output-buffer :wk "Recenter output")
      "x k" '(TeX-kill-job :wk "Kill compile job"))
    ...
    (use-package pdf-tools
      ...
      (auto-revert-verbose nil)     ; no "Reverting buffer" chatter on recompile
      :hook (pdf-view-mode . auto-revert-mode) ; live-update: redisplay when
    										; latexmk rewrites the PDF
      :config
      (pdf-tools-install))
#+end_src
...
*** Swift (Darwin only)
Swift development setup for iOS/tvOS development.
Uses the system Swift toolchain for LSP and build tools since nixpkgs Swift (5.10) is outdated compared to current (6.2).
This configuration only loads on Darwin (macOS) systems.

*Installation Notes:*
- =swift-mode= and =swift-ts-mode= are provided by Nix (syntax highlighting, basic support)
- =swift-development= is installed via the custom overlay from GitHub
  - Provides Xcode-like IDE features: simulator support, device management, project creation
  - Repository: https://github.com/konrad1977/swift-development
- All Swift tooling (sourcekit-lsp, swift, xcodebuild) uses system installations (via Xcode)

*System Requirements:*
- Xcode (full installation required, not just Command Line Tools)
- =xcode-select --install= for command-line tools
- =brew install xcode-build-server= for LSP integration

#+begin_src emacs-lisp
(when (eq system-type 'darwin)
  ;; Swift mode for syntax highlighting
  (use-package swift-mode
    :mode "\\.swift\\'"
    :hook (swift-mode . (lambda ()
                          ;; Use system Swift tools (newer than nixpkgs)
                          ;; LSP will be handled by lsp-mode using system sourcekit-lsp
                          (setq-local lsp-sourcekit-executable
                                      (or (executable-find "sourcekit-lsp")
                                          "/usr/bin/sourcekit-lsp"))))
    :custom
    (swift-mode:basic-offset 4))

  ;; swift-development: Xcode-like IDE features for Swift development
  ;; https://github.com/konrad1977/swift-development
  ;; Installed via custom overlay from flake input
  (use-package swift-development
    :demand nil  ;; Don't load immediately, only when needed
    :after swift-mode
    :custom
    ;; Use system Swift tools for iOS/tvOS development
    (swift-development:use-sourcekit-lsp t)
    (swift-development:simulator-app-path "/Applications/Xcode.app/Contents/Developer/Applications/Simulator.app")
    :config
    ;; Ensure we use system tools, not nix-provided ones
    (when (executable-find "swift")
      (setq swift-development:swift-executable (executable-find "swift")))
    (when (executable-find "xcodebuild")
      (setq swift-development:xcodebuild-executable (executable-find "xcodebuild")))
    (when (executable-find "xcrun")
      (setq swift-development:xcrun-executable (executable-find "xcrun")))
    (when (executable-find "sourcekit-lsp")
      (setq swift-development:sourcekit-lsp-executable (executable-find "sourcekit-lsp"))))

  ;; Format-All integration for Swift (Darwin only)
  (with-eval-after-load 'format-all
    ;; Use system swiftformat if available, otherwise fall back to nix-provided one
    (setq format-all-formatters
          (cons '("Swift" (swiftformat))
                (assq-delete-all 'Swift format-all-formatters))))

  ;; LSP configuration for Swift
  (with-eval-after-load 'lsp-mode
    ;; Register sourcekit-lsp for Swift files
    (when (require 'lsp-sourcekit nil 'noerror)
      (setq lsp-sourcekit-executable (or (executable-find "sourcekit-lsp")
                                         "/usr/bin/sourcekit-lsp")))))
#+end_src
...
*** Ghostel
Uses libghostty to run a real terminal inside emacs.
#+begin_src emacs-lisp
      (use-package ghostel
        :custom
...
#+begin_src emacs-lisp
    ...
    (defun rdmacs/smart-compile ()
      "Run `project-compile' if in a project, otherwise `compile'."
      (interactive)
      (if (project-current)
...
** Git Timemachine
Navigate through the history of a file via git.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
    (use-package smerge-mode
      ...
      :config
      (rdmacs/leader-keys
        "m" '(:keymap smerge-basic-map :package smerge-mode :wk "Merge conflict handling")))
#+end_src
...
#+begin_src emacs-lisp
    (use-package corfu
      ...
      (global-corfu-minibuffer nil) ;; disable in the minibuffer
    ...
    (use-package nerd-icons-corfu
      ...
      :config
      ;; Work around nerd-icons-corfu referencing the undefined
      ;; `elisp-feature' face for module completion candidates.
      (setcdr (assq 'module nerd-icons-corfu-mapping)
              '(:style "cod"
                :icon "file_submodule"
...
* AI
** Agent Shell
Better native claude code integration into emacs
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
    ...
    (defun rdmacs/org-font-setup ()
      ...
      (dolist (face '((org-level-1 . 1.2)
                      ...
                      (org-level-7 . 1.1)
                      (org-level-8 . 1.1)))
        (set-face-attribute (car face) nil :font "SFPro Nerd Font" :weight 'regular :height (cdr face)))
...
**  Org Knowledge Base
Configure the canonical private Org repository when it is present on the
current machine. Agenda discovery is recursive so tasks in project and roadmap
files are visible. Templates and archives remain inert. Machines without the
checkout continue to start normally.

#+begin_src emacs-lisp
(defconst rdmacs/org-notes-directory
  (expand-file-name "~/Documents/Org/")
  "Root of the private Org knowledge base.")

(defun rdmacs/org-notes-agenda-file-p (file)
  "Return non-nil when FILE should contribute to Org Agenda."
  (let ((relative (file-relative-name file rdmacs/org-notes-directory)))
    (not (or (string-match-p
              "\\(?:\\`\\|/\\)\\(?:templates\\|archive\\|\\.git\\)/"
              relative)
             (member relative '("README.org" "index.org" "setup.org"))))))

(defun rdmacs/org-notes-files ()
  "Return current recursive Agenda files for the private knowledge base."
  (when (file-directory-p rdmacs/org-notes-directory)
    (seq-filter #'rdmacs/org-notes-agenda-file-p
                (directory-files-recursively
                 rdmacs/org-notes-directory "\\.org\\'"))))

(defun rdmacs/refresh-org-agenda-files (&rest _)
  "Refresh nested Agenda files before each Agenda invocation."
  (setq org-agenda-files (rdmacs/org-notes-files)))

(defun rdmacs/org-notes-open-index ()
  "Open the private knowledge-base index."
  (interactive)
  (find-file (expand-file-name "index.org" rdmacs/org-notes-directory)))

(defun rdmacs/org-notes-search ()
  "Search the private Org knowledge base with ripgrep."
  (interactive)
  (consult-ripgrep rdmacs/org-notes-directory))

(when (file-directory-p rdmacs/org-notes-directory)
  (let ((inbox (expand-file-name "inbox.org" rdmacs/org-notes-directory))
        (journal (expand-file-name "journal.org" rdmacs/org-notes-directory)))
    (setq org-directory rdmacs/org-notes-directory
          org-preview-latex-image-directory
          (expand-file-name "~/.cache/emacs/org-latex-preview/")
          ...
          `(("t" "Task" entry
             (file+headline ,inbox "Unprocessed")
             "* TODO %?\n  %U\n" :empty-lines 1)
            ("n" "Inbox note" entry
             (file+headline ,inbox "Unprocessed")
             "* %?\n  %U\n" :empty-lines 1)
            ("j" "Journal" entry
             (file+olp+datetree ,journal "Journal")
             "* %U %?\n" :tree-type day :empty-lines 1))))
  (rdmacs/refresh-org-agenda-files))

(unless (advice-member-p #'rdmacs/refresh-org-agenda-files 'org-agenda)
  (advice-add 'org-agenda :before #'rdmacs/refresh-org-agenda-files))
...
(rdmacs/leader-keys
  ...
  "a i" '(rdmacs/org-notes-open-index :wk "Notes index")
  "a s" '(rdmacs/org-notes-search :wk "Search notes")
  "a l" '(org-store-link :wk "Store link")
  "a e" '(org-export-dispatch :wk "Export"))
#+end_src
...
** Org Babel
This allows creating files from org mode source blocks. Some languages have additional setup required.
...
#+begin_src emacs-lisp
    (use-package consult
      ...
      ;; Make consult buffer aware of project files as a fallback
      (defvar consult--source-project-file
      	`(:name "Project File"
                 ...
                 (when project
      			...
      			"project.el file source for `consult-buffer'."))
      (add-to-list
       'consult-buffer-sources
       'consult--source-project-file :append)
...
#+begin_src emacs-lisp
        (use-package consult-dir
          ...
          :config
          (setf (plist-get consult-dir--source-recentf :action)
                (lambda (cand)
                  (let ((default-directory cand))
                    (consult-dir-dired)))
                (plist-get consult-dir--source-project :action)
                (lambda (cand)
                  (let ((default-directory cand))
                    (consult-dir-dired)))
                (plist-get consult-dir--source-bookmark :action)
                (lambda (cand)
                  (let ((default-directory (bookmark-get-filename cand)))
                    (consult-dir-dired))))
          (add-to-list 'consult-buffer-sources 'consult-dir--source-recentf 'append)
          (add-to-list 'consult-buffer-sources 'consult-dir--source-bookmark 'append)
          (add-to-list 'consult-buffer-sources 'consult-dir--source-project 'append))
#+end_src
...
** Treemacs
The `treemacs' package provides a file tree explorer for Emacs, allowing easy navigation
...
#+begin_src emacs-lisp
    (use-package treemacs
      ...
      :defer t
      :custom
      (treemacs-show-hidden-files t)                ;; By default shows hidden files
      (treemacs-git-mode 'deferred)                 ;; Git integration with deferred mode for performance
      (treemacs-follow-after-init t)                ;; Follow current file after startup
      (treemacs-width 35)                           ;; Default width
      :config
      (treemacs-hide-gitignored-files-mode nil))    ;; Show gitignored files
...
** Envrc
Integrate Direnv with Emacs via =envrc=.  We use =envrc-global-mode= so that
all buffers (including =*compilation*=) inherit the direnv environment, but
we restrict it to only the major modes that actually need it.  This avoids the
performance hit of running =envrc--find-env-dir= (via =locate-dominating-file=)
on every transient special-mode buffer (Magit, Help, tabulated-list, etc.).
#+begin_src emacs-lisp
    (use-package envrc
      :init
      ;; Only enable envrc in buffers that actually visit files or run
      ;; external processes.  This prevents the expensive direnv lookup
      ;; from running on every transient buffer (magit, help, etc.).
      (setq envrc-global-modes
            '(prog-mode            ; all programming modes
              text-mode            ; org, markdown, etc.
              compilation-mode     ; *compilation* buffers
              comint-mode          ; shell, inferior processes
              eshell-mode
              eat-mode
              term-mode
              shell-mode
              ghostel-mode         ; ghostel-compile buffers during the run
              (not special-mode)   ; exclude help, magit, etc.
              dired-mode))         ; re-enable dired (derived from special-mode)
      (envrc-global-mode)
      :config
      ;; `compilation-start' copies `process-environment' from the calling
      ;; buffer.  If the calling buffer is excluded by `envrc-global-modes'
      ;; (e.g. `magit-status'), `buffer-env' is nil and `compilation-start'
      ;; kills the envrc environment that `envrc-global-mode' set in the
      ;; *compilation* buffer.  This advice temporarily applies envrc to the
      ;; calling buffer so `compilation-start' copies the correct environment.
      ;;
      ;; NOTE: this is a fallback for non-ghostel compile (e.g. `grep-mode',
      ;; which `ghostel-compile-global-mode' excludes).  When
      ;; `ghostel-compile-global-mode' is active, it advises
      ;; `compilation-start' and routes to `ghostel-compile--start', which
      ;; captures `process-environment' from the *ghostel* buffer itself —
      ;; not the calling buffer.  That is why `ghostel-mode' is in
      ;; `envrc-global-modes' above.
      (defun envrc-propagate-compilation (orig &rest args)
        "Ensure `compile' respects envrc even when called from a non-envrc buffer."
        (if (or envrc-mode
                (not (envrc--find-env-dir)))
            (apply orig args)
          (envrc--update)
          (unwind-protect
              (apply orig args)
            (envrc--clear (current-buffer)))))
      (advice-add 'compile :around #'envrc-propagate-compilation)
      (setq compilation-environment
            (cons "__NIX_DARWIN_SET_ENVIRONMENT_DONE=1"
                  (delete "__NIX_DARWIN_SET_ENVIRONMENT_DONE=1"
