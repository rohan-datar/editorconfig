...
#+begin_src emacs-lisp
    ...
    (use-package latex
      ...
      :config
      ;; Use the latexmk build driver AUCTeX 14 ships ("LaTeXMk"). AUCTeX resets
      ;; TeX-command-default to "LaTeX" per buffer on mode entry, so set it in
      ;; the hook rather than globally. The chktex flymake backend is registered
      ;; by AUCTeX itself; chktex comes from the texlive toolchain.
