...
#+begin_src emacs-lisp
    (let ((cache-dir (expand-file-name "~/.cache/emacs/")))
      ...
      ;; project.el known projects list
      ...
      ;; Shell-maker (used by agent-shell, chatgpt-shell, etc.)
      ...
      ;; github tokens for gtpel
      (setq gptel-gh-token-file (expand-file-name "copilot-chat/token" cache-dir))
      (setq gptel-gh-github-token-file (expand-file-name "copilot-chat/github-token" cache-dir)))
...
#+begin_src emacs-lisp
    (use-package general
      ...
      ;; ghostel is broken on darwin; fall back to vterm there
...
Broken on darwin (libghostty issue), so darwin falls back to vterm below.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
    (use-package gptel
      :config
      (setq gptel-model 'claude-4.5-opus
            gptel-backend (gptel-make-gh-copilot "Copilot"))
      (rdmacs/leader-keys
    	"a g" '(:keymap gptel-mode-map :package gptel :wk "gptel")))
#+end_src
...
#+begin_src emacs-lisp
    ...
    (use-package claude-code
      :config
      (setq claude-code-terminal-backend (if (eq system-type 'darwin) 'vterm 'ghostel))
      ...
      (claude-code-mode)
      (rdmacs/leader-keys
    	"a c" '(:keymap claude-code-command-map :package claude-code :wk "Claude Code")))
#+end_src
